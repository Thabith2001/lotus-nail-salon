...
const Services_admin = () => {
    ...
    const fetchData = async (type) => {
        try {
            ...
            if (type === 'services') items = resp.data?.services || [];
            else if (type === 'packages') items = resp.data?.packages || [];
            else if (type === 'membership') items = resp.data?.memberships || [];
    ...
    const stats = useMemo(() => {
        return {
            total: data.length,
            recommended: data.filter(d => d.recommended).length,
    ...
    const filteredData = useMemo(() => {
        const term = (searchTerm || localSearch).toLowerCase();
        ...
        return filtered;
    }, [data, searchTerm, localSearch, sortBy]);
    ...
    const handleFormSubmit = async () => {
        try {
            if (modalMode === 'add') {
                // Add new item
                const response = await axios.post(`/api/${activeTab}`, formData);
                toast.success(`${activeTab.slice(0, -1)} added successfully!`, {
                    style: {background: '#10b981', color: '#fff'},
                });
            } else {
                // Update existing item
                const response = await axios.put(`/api/${activeTab}/${selectedItem.id}`, formData);
        ...
        return (
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    {item.sessions && (
                        <span className="px-2 py-1 text-xs bg-orange-500/20 text-orange-300 rounded-lg">
                ...
                {/* Features */}
                {item.features?.length > 0 && item.features[0] !== '' && (
                ...
                {/* Benefits */}
                {item.benefits?.length > 0 && item.benefits[0] !== '' && (
                    <div className="pt-2 border-t border-white/10">
    ...
    return (
        <div className="min-h-screen bg-white/5 border-white/10 rounded-2xl p-4 md:p-8">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl space-y-4">
                    ...
                    {/* Tabs */}
                    <div className="flex flex-wrap gap-2">
                        {['services', 'packages', 'membership'].map((tab) => (
                            <button
                                key={tab}
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 bg-black/80 rounded-2xl backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-gradient-to-r from-purple-950/80 to-via-fuchsia-600/75 rounded-2xl border border-white/10 w-full max-w-md shadow-2xl">
                        {/* Modal Header */}
                        ...
                        <div className="p-6">
                            <p className="text-gray-300">
                                Are you sure you want to delete <span className="font-semibold text-white">&#34;{selectedItem?.name}&#34;</span>? This will permanently remove this item from your system.
                            </p>
                        </div>
